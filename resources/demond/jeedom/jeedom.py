...
class jeedom_com(object):
    ...
    def send_changes_async(self):
        try:
            if len(self.changes) == 0:
                resend_changes = threading.Timer(self.cycle,
                                                 self.send_changes_async)
            ...
            try:
                ...
                logging.error(
                    'Error on send request to jeedom %s', str(error))
            dt = datetime.datetime.now() - start_time
            seconds = dt.days * 86400 + dt.seconds
            ms = seconds * 1000 + dt.microseconds / 1000.0
            timer_duration = self.cycle - ms
            if timer_duration < 0.1:
                timer_duration = 0.1
            resend_changes = threading.Timer(timer_duration,
                                             self.send_changes_async)
            resend_changes.start()
        except Exception as error:
            logging.error(
                'Critical error on  send_changes_async %s' % (str(error),))
            resend_changes = threading.Timer(self.cycle,
                                             self.send_changes_async)
            resend_changes.start()
    ...
    def send_change_immediate(self, change):
        try:
            ...
            r = requests.post(self.url + '?apikey=' + self.apikey, json=change,
                              timeout=(0.5, 120), verify=False)
            if r.status_code != requests.codes.ok: #pylint: disable=no-member
                logging.error(
                    'Error on send request to jeedom, return code %s',
                    str(r.status_code))
    ...
    def test(self):
        try:
            response = requests.get(self.url + '?apikey=' + self.apikey,
                                    verify=False)
...
class jeedom_utils(object):
    ...
    def set_log_level(level='error'):
        ...
        logging.basicConfig(level=jeedom_utils.convert_log_level(level),
    ...
    def find_tty_usb(idVendor, idProduct, product=None):
        ...
        for device in context.list_devices(subsystem='tty'):
            ...
            if product is not None:
                if 'ID_VENDOR' not in device or \
                        device['ID_VENDOR'].lower().find(product.lower()) == -1:
                    continue
...
class jeedom_serial(object):
    ...
    def open(self):
        if self.device:
            logging.debug("Open serial port on device: " + str(
                self.device) + ', rate ' + str(
                self.rate) + ', timeout : ' + str(self.timeout))
        else:
            logging.error("Device name missing.")
            return False
        logging.debug("Open Serialport")
        try:
            self.port = serial.Serial(self.device, self.rate,
                                      timeout=self.timeout)
        except serial.SerialException, e:
            logging.error(
                "Error: Failed to connect on device " + self.device + " Details : " + str(
                    e))
            return False
    ...
    def close(self):
        ...
        try:
            ...
            logging.error(
                "Failed to close the serial port (" + self.device + ")")
            return False

    def write(self, data):
        logging.debug(
            "Write data to serial port : " + str(jeedom_utils.ByteToHex(data)))
...
class jeedom_socket_handler(StreamRequestHandler):
    def handle(self):
        ...
        logging.debug("Client connected to [%s]", self.client_address[0])
        lg = self.rfile.readline()
        JEEDOM_SOCKET_MESSAGE.put(lg)
        logging.debug("Message read from socket: " + lg.strip())
        self.netAdapterClientConnected = False
        logging.debug("Client disconnected from [%s]", self.client_address[0])


class jeedom_socket(object):
